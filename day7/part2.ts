...
function getHandType(hand: string): THand {
  ...
  // sort them to get the card with max occurrences
  const sortedOccurrences = Object.entries(occurrences).sort(([_, v1], [__, v2]) => (v1 > v2 ? -1 : 1));
  if (hand.includes('J')) {
    const maxCard = sortedOccurrences[0][0];
    if (maxCard === 'J' && sortedOccurrences.length > 1) {
      occurrences[sortedOccurrences[1][0]] += hand.match(/J/g)!.length;
      delete occurrences['J'];
    } else if (maxCard !== 'J') {
      occurrences[maxCard] += hand.match(/J/g)!.length;
      delete occurrences['J'];
    }
  }
